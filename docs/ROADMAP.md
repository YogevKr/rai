...
- Native macOS notifications on blocked → done; click → raise rai + focus pane.
- Dock badge / menu-bar count of agents needing you.
- Sound + persisted global mute; per-pane do-not-disturb remains a follow-up.

Implementation note: `RaiApp` constructs one shared `RaiModel` and gives
...
authoritative protocol. Structural ops rai drives via the `herdr` CLI
