...
Pane attach starts the live full-grid stream first, then reads history in the background.
The Mac skips the visible-grid preview for clients that support fast attach.
The stream's first full frame paints the current session while history loads.
History can arrive before or after that frame without adding a second seam.

The phone ignores legacy visible-grid previews because they differ in styling and cursor position.
